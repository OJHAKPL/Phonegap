	...
	function homeLogin() {
		$.post(
		"http://www.nd2nosmart.cards/nd2no/admin/web-login",
		{
		...
		function(data,status){
			...
			$.each(dataArray, function(i, field){	
				...
				if(field.email){
					...
					$.post(
						"http://www.nd2nosmart.cards/nd2no/admin/web-show-folders",
						{
						  user_id: field.id
						},
	...
	function showFoldercards(folderId) {
		...
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-folder-cards",
			{
			  folder_id: folderId,
			},
	...
	function deleteMyfolder(card_shared_id){
		...
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-folder-delete",
			{
	...
	function cartDetailsNotification(cardId) {
		...
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-cards-detail",
			{
			  id: cardId
			},
		...
		if(user_id){
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-reset-notification",
				{
					card_id: cardId,
					user_id: user_id
				},
	...
	function sharedcardlist(){
		...
		if(user_id){
			...
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-shared-cards",
				{
				  user_id: user_id,
				},
				function(cardlist,status){
					...
					if(!cardlistArr.error) {
						$.each( cardlistArr, function(i, row1) {
							$.each( row1, function(i, row) {
								...
								$('.cardslistHtml').append('<div class="card-box allcardlist"><div class="card-option-open"><a href="javascript:void(0);" class="tick-button ui-link"><img onClick="cartDetails('+row.id+');" src="images/eye-icon.png" alt=""></a><a href="javascript:void(0);" onClick="addCart('+row.id+');" class="tick-button ui-link"><img class="cardclass_'+row.id+'" src="'+tickiconImg+'" alt=""></a> <a href="javascript:void(0);" class="tick-button ui-link"><img onClick="deleteCard('+row.card_shared_id+');" src="images/delete-icon.png" alt=""></a></div><div class="img"><img width="100%" src="http://www.nd2nosmart.cards/nd2no/upload/cards/large/'+row.banner+'" alt=""></div></div>');
							});
						});
	...
	function deleteCard(card_shared_id) { 
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-remove-shared-card",
			{
				shared_id: card_shared_id,
			},
			function(result, status){
	...
	function removeSelected(cardId) {
		...
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-selected-cards",
			{
			...
			if(!cardlistArr.error) {
					$.each( cardlistArr, function(i, row1) {
						$.each( row1, function(i, row) {				
							$('.ticked-list').append('<li class="Allselectedcards"><div class="card-option"><a href="javascript:void(0);" class="tick-button ui-link"><img onClick="removeSelected('+row.id+');" src="images/delete-icon.png" alt=""></a></div><div class="img"><img src="http://www.nd2nosmart.cards/nd2no/upload/cards/large/'+row.banner+'" alt=""></div></li>');
	...
	function editCard(cardId){
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-update-card",{
				card_id: cardId,
			},
			function(profiledetails, status){
				...
				if(!profileArr.error) {
					$.each( profileArr, function(i, row) {
						...
						var banner			= (row.banner && row.banner!='')?'http://www.nd2nosmart.cards/nd2no/upload/cards/large/'+row.banner+'':'images/card-thumb.png';
	...
	function shareEmailCheck(){
		...
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-share-cards",
			{
			  email: email,
	...
	function viewProfile(){
		...
		if(user_id){
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-user-info",
				{
				  id: user_id,
				},
				function(profiledetails,status){
					...
					if(!profileArr.error) {
						$.each( profileArr, function(i, row) {
							...
							var photoUser   = (row.photo && row.photo!='')?'http://www.nd2nosmart.cards/nd2no/upload/users/profile-photo/resized/'+row.photo+'':'images/blank_profile.png';
			...
			function(profiledetails,status){
				...
				if(!profileArr.error) {
					$.each( profileArr, function(i, row) {
						...
						$.post(
							"http://www.nd2nosmart.cards/nd2no/admin/select-countries-data",
	...
	function EditProfileSubmit(){
		...
		$('#editprofile').validate({
			...
			submitHandler:function (form) {
				...
				$.post(
					"http://www.nd2nosmart.cards/nd2no/admin/web-update-profile",
	...
	function favoritelist(){
		...
		if(user_id){
			...
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-show-favourites",
				{
				  user_id: user_id,
				},
				function(cardlist,status){
	...
	function removeFavorite(card_favourite_id) { 
		$.post(
			"http://www.nd2nosmart.cards/nd2no/admin/web-remove-favourite-card",
			{
	...
	function cardlist(){
		...
		if(user_id){
			...
			$('.cardlistloader').show();
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-user-cards",
				{
				  user_id: user_id,
				},
				function(cardlist,status){
					...
					if(!cardlistArr.error) {
						$.each( cardlistArr, function(i, row1) {
							$.each( row1, function(i, row) {
								$('.cardslistHtml').append('<div class="card-box allcardlist"><div class="card-option-open"><a href="javascript:void(0);" class="tick-button ui-link"><img onClick="cartDetails('+row.id+');" src="images/eye-icon.png" alt=""></a><a href="javascript:void(0);" class="tick-button ui-link"><img onClick="editCard('+row.id+');" src="images/edit-icon.png" alt=""></a></div><div class="img"><img width="100%" src="http://www.nd2nosmart.cards/nd2no/upload/cards/large/'+row.banner+'" alt=""></div></div>');
							});
						});
						$('.cardlistloader').hide();
					} else {
	...
	function moveTocard(){
		...
		if(user_id){	
			...
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-folder-list",
				{
				  user_id: user_id,
				},
				function(folderlist,status){
	...
	function moveTofoldersave(){
		...
		if(user_id){
			...
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-folder-moveto",
				{
				  user_id: user_id,
	...
	$(document).ready(function(){
		...
		$('#register_form').validate({
			...
			submitHandler:function (form) {				
				$.post(
					"http://www.nd2nosmart.cards/nd2no/admin/web-register",
		...
		$("#folder_save").click(function(){	
			...
			if(user_id){
				...
				$.post(
					"http://www.nd2nosmart.cards/nd2no/admin/web-create-folder",
					{
					  user_id: user_id,
		...
		$("#forgot-email-button").click(function(){
			$.post(
				"http://www.nd2nosmart.cards/nd2no/admin/web-forget-password",
				{
		...
		$(document).on('pagebeforeshow', function(){
			...
			if(user_id){
				$.post(
					"http://www.nd2nosmart.cards/nd2no/admin/web-notification-total",
					{
					  user_id: user_id
					},
		...
		$(".counter-notify").click(function(){
			...
			if(user_id){
				...
				$.post(
					"http://www.nd2nosmart.cards/nd2no/admin/web-notification-list",
					{
					  user_id: user_id,
					},
					function(cardlist,status){
		...
		$('#changepassword_form').validate({
			...
			submitHandler:function (form) {	
				...
				if(user_id){		
					$.post(
						"http://www.nd2nosmart.cards/nd2no/admin/web-cahnge-password",
						{
